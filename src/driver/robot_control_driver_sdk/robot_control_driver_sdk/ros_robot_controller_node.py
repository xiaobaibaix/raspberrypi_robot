...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        ...
        self.create_service(GetButtonState, '~/get_button', self.get_button_state)
    ...
    def get_button_state(self,request, response):
        ress=response.state
        while True:
            state=self.board.get_button()
            res=ButtonState()
            if state is not None:
                self.get_logger().info(f"get_button_state:({state[0]}{state[1]})")
                res.id=state[0]
                res.state=state[1]
                ress.append(res)
            else:   
                self.get_logger().info(f"get_button_state:null")
                res.id=10
                res.state=10
                ress.append(res)
                break
