...
namespace namespace mpu6050_drive_broadcaster
{

CallbackReturn MPU6050DriverBroadcaster::on_init(const hardware_interface::HardwareInfo & info)
{
...
MPU6050DriverBroadcaster::export_state_interfaces()
{
...
CallbackReturn MPU6050DriverBroadcaster::on_activate(const rclcpp_lifecycle::State &)
{
...
CallbackReturn MPU6050DriverBroadcaster::on_deactivate(const rclcpp_lifecycle::State &)
{
...
hardware_interface::return_type MPU6050DriverBroadcaster::read(const rclcpp::Time &,
...
bool MPU6050DriverBroadcaster::i2c_write(uint8_t reg, uint8_t val)
{
  uint8_t out[2] = {reg, val};
  return ::write(i2c_fd_, out, 2) == 2;
}

bool MPU6050DriverBroadcaster::i2c_read(uint8_t reg, uint8_t * buf, size_t len)
{
  if (::write(i2c_fd_, &reg, 1) != 1) return false;
  return ::read(i2c_fd_, buf, len) == static_cast<ssize_t>(len);
}

void MPU6050DriverBroadcaster::complementary_filter(double dt)
{
  // 加速度计算倾角
...
PLUGINLIB_EXPORT_CLASS(mpu6050_drive_broadcaster::MPU6050DriverBroadcaster, hardware_interface::SensorInterface)